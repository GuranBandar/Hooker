...
namespace Hooker_GUI
{
    ...
    {
        private DataSet _puttstatistikDS = new DataSet();
        ...
        {
            txtAntal.Text = _puttstatistikDS.Tables["Antal"].Rows[0]["Antal rundor"].ToString();

            foreach (DataRow rad in _puttstatistikDS.Tables["Puttstatistik"].Rows)
            {
                switch (rad["Par"].ToString())
                {
                    ...
                    case "9":
                        ...
                        txtTotaltPuttsnitt.Text = ("ND1").Formatera(
                            int.Parse(rad["Antal puttar"].ToString())
                            / decimal.Parse(_puttstatistikDS.Tables["Antal"].Rows[0]["Antal rundor"].ToString()));
        ...
        {
            fönsterhanterare1.HanteraVisaAndelgraf(_puttstatistikDS);
        }
        ...
        private void knappkontroller1_OnKnapp3Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    _puttstatistikDS = statistik.Puttstatistik(golfklubbNr, banaNr, spelarID.ToString(), fromDatum, tomDatum,
                        hcprond, niohalsrond, sallskapsrond, tavlingsrond);

                    if (_puttstatistikDS.Tables["Antal"].Rows[0]["Antal rundor"].ToString() != "0")
                    {
