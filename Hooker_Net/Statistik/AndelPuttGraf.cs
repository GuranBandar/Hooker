    ...
    public partial class AndelPuttGraf : FormBas
    {
        ...
        public void VisaAndelgraf(DataSet andelDS)
        {
            foreach (DataRow rad in andelDS.Tables["Puttstatistik"].Rows)
            {
                ...
                {
                    case "9":
                        decimal andelPutt = Convert.ToDecimal(rad["Antal puttar"]) /
                            (Convert.ToDecimal(rad["Antal puttar"]) +
                            Convert.ToDecimal(rad["Antal slag"]));
                        decimal andelSlag = Convert.ToDecimal(rad["Antal slag"]) /
                            (Convert.ToDecimal(rad["Antal puttar"]) +
                            Convert.ToDecimal(rad["Antal slag"]));
